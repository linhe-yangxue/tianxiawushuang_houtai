...
<div class="container clearfix">
    ...
    <div class="main-wrap">
                ...
                <div class="result-title">
                    <form action="/account.php" method="get">
                        ...
                        <select class="form-control" id="channelId" name='channelId'>
						    <option value = '0' <?php if( !isset($_GET['channelId'])){ ?> selected="selected" <?php } ?>>选择渠道ID</option>
                            <option value = '10006' <?php if( isset($_GET['channelId']) && (intval($_GET['channelId'])) == 10006){ ?> selected="selected" <?php } ?>>10006</option>
                            <option value = '10007' <?php if( isset($_GET['channelId']) && (intval($_GET['channelId'])) == 10007){ ?> selected="selected" <?php } ?>>10007</option>                            
                            <option value = '99999' <?php if( isset($_GET['channelId']) && (intval($_GET['channelId'])) == 99999){ ?> selected="selected" <?php } ?>>99999</option>
                        </select>&nbsp;
		...
		<?php
		   ...
		   {
		   	if(!isset($_GET['channelId']) || intval($_GET['channelId']) <= 0)
		   	{
				echo "<span style='color:red'>请选择渠道ID</span>";return;
		  	}
		   }
		?>
                <div class="result-content">
                    <table class="result-tab" width="100%" >
                        ...
                        <?php 
							...
							$sql = "select * from uw_account where 1";
							if(isset($_GET['channelId']) && intval($_GET['channelId']) > 0)
							{
